...
export function createTargetWaterV3({width=28,depth=6.2,seed=11,quality='auto'}={}){
  ...
  const uniforms={
    uTime:{value:0},
    uDeep:{value:new THREE.Color(0x063d5e)},
    uMid:{value:new THREE.Color(0x0b7092)},
    uShallow:{value:new THREE.Color(0x4fb8b5)},
    uSky:{value:new THREE.Color(0x7896a2)},
    uSun:{value:new THREE.Color(0xffe0a6)},
    uSunDir:{value:new THREE.Vector3(-.34,.88,.31).normalize()},
    uReflectionStrength:{value:practical?.20:.36},
    uSparkleStrength:{value:practical?.38:.62},
    uWaveStrength:{value:practical?.68:.94},
    uCausticStrength:{value:practical?.18:.32},
    uFoamStrength:{value:practical?.62:1.00},
    uDepthContrast:{value:practical?1.12:1.32},
  ...
  // v47 quality regate: fragment alpha follows a deterministic irregular bank profile on the
  // narrow axis. This removes the prior obvious rectangular river silhouette without changing
  // traversal/collision authority or requiring a second render pass.
  ...
  g.userData={assetId:'water_target_v3',version:'3.10.0',stage:'M07_OWNER_REGATE_V50',source:'repo-procedural',qualityPreset:resolvedQuality,reflectionMethod:'single-pass analytic wave-normal Fresnel sky reflection + directional sun specular; no planar RTT/SSR',performanceIntent:practical?'52x20 surface grid, reduced sparkle/caustic/shore foam, fewer rocks, no rock cast shadows':'96x40 surface grid, denser sparkle/caustic/shore foam, full shoreline rock shadows',resourceBudget:{surfaceSegments:[segmentsX,segmentsY],sparkleCount:sparkCount,rockCount,rockCastShadows:!practical,reflectionStrength:uniforms.uReflectionStrength.value,waveStrength:uniforms.uWaveStrength.value,causticStrength:uniforms.uCausticStrength.value,foamStrength:uniforms.uFoamStrength.value},intent:'target-derived river with deterministic irregular bank clipping, deeper basin contrast, readable turquoise shallows, broken shoreline foam and sky/sun response while retaining restrained non-emissive caustics, moving reflection and grounded rocks; removes the rectangular river-edge read exposed by the Owner quality regate without changing traversal authority',update:t=>uniforms.uTime.value=t};
  return g;
}
