...
export function createLukeCharacter({scale=1}={}){
  ...
  const hips=new THREE.Group();hips.position.y=.50*scale;g.add(hips);
  ...
  const shoulderGeo=new THREE.SphereGeometry(.145*scale,12,9);for(const x of[-.36,.36]){const s=new THREE.Mesh(shoulderGeo,navy);s.scale.set(1.16,.78,.90);s.position.set(x,.90*scale,0);hips.add(s)}const mantle=new THREE.Mesh(new THREE.BoxGeometry(.58*scale,.30*scale,.075*scale),navy);mantle.position.set(0,.82*scale,.19*scale);mantle.rotation.x=.08;hips.add(mantle);const mantleTrim=new THREE.Mesh(new THREE.BoxGeometry(.50*scale,.045*scale,.085*scale),gold);mantleTrim.position.set(0,.69*scale,.205*scale);hips.add(mantleTrim);const backCrest=new THREE.Mesh(new THREE.OctahedronGeometry(.105*scale,0),gold);backCrest.scale.set(.72,1.10,.28);backCrest.position.set(0,.83*scale,.235*scale);hips.add(backCrest);const sashMat=new THREE.MeshStandardMaterial({color:0x174d9b,roughness:.70});for(const x of[-.13,.13]){const sash=new THREE.Mesh(new THREE.BoxGeometry(.15*scale,.50*scale,.055*scale),sashMat);sash.position.set(x,.18*scale,.19*scale);sash.rotation.x=-.10;sash.rotation.z=x<0?-.07:.07;hips.add(sash)}
  g.userData={assetId:'luke_player_v1',version:'1.3.0',stage:'M08_TARGET_PRESENCE_V2',source:'repo-procedural',rig:'procedural hierarchical pivots',intent:'stylized blue-haired JRPG hero with readable coat silhouette, crest, layered hair, hands and boots; M08 separates subtle idle breathing from locomotion swing to prevent full-amplitude slow-motion foot sliding',animate:(t,movement=1)=>{const move=THREE.MathUtils.clamp(movement,0,1),walkPhase=t*(6.8+move*1.4),idlePhase=t*2.15,swing=Math.sin(walkPhase)*.48*move,breath=Math.sin(idlePhase);armL.rotation.x=swing+breath*.018*(1-move);armR.rotation.x=-swing-breath*.018*(1-move);legL.rotation.x=-swing*.72;legR.rotation.x=swing*.72;hips.position.y=(.50+.024*Math.abs(Math.sin(walkPhase))*move+.007*breath*(1-move))*scale;hips.rotation.z=Math.sin(walkPhase)*.012*move;hips.rotation.x=breath*.006*(1-move)}};
  return shadow(g)
}
