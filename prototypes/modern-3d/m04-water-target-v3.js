...
export function createTargetWaterV3({width=28,depth=6.2,seed=11,quality='auto'}={}){
  ...
  const uniforms={
    uTime:{value:0},
    uDeep:{value:new THREE.Color(0x043f69)},
    uMid:{value:new THREE.Color(0x096f95)},
    uShallow:{value:new THREE.Color(0x40aeb7)},
    uSky:{value:new THREE.Color(0x86a6b8)},
    uSun:{value:new THREE.Color(0xffdda3)},
    uSunDir:{value:new THREE.Vector3(-.34,.88,.31).normalize()},
    uReflectionStrength:{value:practical?.17:.30},
    uSparkleStrength:{value:practical?.27:.50},
    uWaveStrength:{value:practical?.68:.94},
    uCausticStrength:{value:practical?.10:.16},
    uFoamStrength:{value:practical?.43:.65},
    uDepthContrast:{value:practical?.94:1.08}
  };
  ...
  g.add(shoreRibbon(width,depth*.47,seed+1,practical?.40:.58),shoreRibbon(width,-depth*.47,seed+2,practical?.40:.58));
  ...
  g.userData={assetId:'water_target_v3',version:'3.8.1',stage:'M07_IN_PROGRESS',source:'repo-procedural',qualityPreset:resolvedQuality,reflectionMethod:'single-pass analytic wave-normal Fresnel sky reflection + directional sun specular; no planar RTT/SSR',performanceIntent:practical?'52x20 surface grid, reduced sparkle/caustic/shore foam, fewer rocks, no rock cast shadows':'96x40 surface grid, denser sparkle/caustic/shore foam, full shoreline rock shadows',resourceBudget:{surfaceSegments:[segmentsX,segmentsY],sparkleCount:sparkCount,rockCount,rockCastShadows:!practical,reflectionStrength:uniforms.uReflectionStrength.value,waveStrength:uniforms.uWaveStrength.value,causticStrength:uniforms.uCausticStrength.value,foamStrength:uniforms.uFoamStrength.value},intent:'target-derived blue river balancing readable basin depth with white broken highlights, shoreline foam, restrained moving reflection, non-emissive shallow caustics and grounded rocks; corrected the prior cyan-flat and over-dark iterations',update:t=>uniforms.uTime.value=t};
  return g;
}
