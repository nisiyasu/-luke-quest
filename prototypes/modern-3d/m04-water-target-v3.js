...
export function createTargetWaterV3({width=28,depth=6.2,seed=11,quality='auto'}={}){
  ...
  const uniforms={
    uTime:{value:0},
    // v3.11 visual regate: lift the basin out of near-black navy. The minimum reference reads
    // as a luminous blue river even in shadow, so keep depth while preserving visible cyan body color.
    uDeep:{value:new THREE.Color(0x075978)},
    uMid:{value:new THREE.Color(0x1289aa)},
    uShallow:{value:new THREE.Color(0x65c8cb)},
    uSky:{value:new THREE.Color(0xa3c0c7)},
    ...
    uSparkleStrength:{value:practical?.48:.76},
    uWaveStrength:{value:practical?.70:.96},
    uCausticStrength:{value:practical?.23:.40},
    uFoamStrength:{value:practical?.72:1.10},
  ...
  g.userData={assetId:'water_target_v3',version:'3.11.0',stage:'M04_REGATE',source:'repo-procedural',qualityPreset:resolvedQuality,reflectionMethod:'single-pass analytic wave-normal Fresnel sky reflection + directional sun specular; no planar RTT/SSR',performanceIntent:practical?'52x20 surface grid, reduced sparkle/caustic/shore foam, fewer rocks, no rock cast shadows':'96x40 surface grid, denser sparkle/caustic/shore foam, full shoreline rock shadows',resourceBudget:{surfaceSegments:[segmentsX,segmentsY],sparkleCount:sparkCount,rockCount,rockCastShadows:!practical,reflectionStrength:uniforms.uReflectionStrength.value,waveStrength:uniforms.uWaveStrength.value,causticStrength:uniforms.uCausticStrength.value,foamStrength:uniforms.uFoamStrength.value},intent:'target-derived luminous blue river with deterministic irregular bank clipping, readable turquoise shallows, stronger whitewater/sparkle cues and preserved depth; v3.11 removes the near-black basin read visible in integrated portrait evidence while retaining traversal authority and single-pass performance',update:t=>uniforms.uTime.value=t};
  return g;
}
