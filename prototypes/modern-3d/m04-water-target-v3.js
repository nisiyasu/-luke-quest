...
export function createTargetWaterV3({width=28,depth=6.2,seed=11,quality='auto'}={}){
  ...
  const uniforms={
    uTime:{value:0},
    uDeep:{value:new THREE.Color(0x04364f)},
    uMid:{value:new THREE.Color(0x08798d)},
    uShallow:{value:new THREE.Color(0x5bcab0)},
    uSky:{value:new THREE.Color(0xa8c7c8)},
    uSun:{value:new THREE.Color(0xffdda3)},
    uSunDir:{value:new THREE.Vector3(-.34,.88,.31).normalize()},
    uReflectionStrength:{value:practical?.18:.32},
    uSparkleStrength:{value:practical?.23:.39},
    uWaveStrength:{value:practical?.68:.94},
    uCausticStrength:{value:practical?.10:.18},
    uFoamStrength:{value:practical?.34:.48},
    uDepthContrast:{value:practical?.84:1.0}
  };
  ...
  g.add(shoreRibbon(width,depth*.47,seed+1,practical?.30:.40),shoreRibbon(width,-depth*.47,seed+2,practical?.30:.40));
  const area=Math.max(1,width*depth),sparkCount=practical?Math.max(80,Math.floor(area*.56)):Math.max(210,Math.floor(area*1.22));
  g.add(sparkleField(width*.95,depth*.87,seed+40,sparkCount,practical?.050:.061,practical?.44:.58));
  ...
  g.userData={assetId:'water_target_v3',version:'3.7.0',stage:'M07_IN_PROGRESS',source:'repo-procedural',qualityPreset:resolvedQuality,reflectionMethod:'single-pass analytic wave-normal Fresnel sky reflection + directional sun specular; no planar RTT/SSR',performanceIntent:practical?'52x20 surface grid, reduced sparkle/caustic/shore foam, fewer rocks, no rock cast shadows':'96x40 surface grid, denser sparkle/caustic/shore foam, full shoreline rock shadows',resourceBudget:{surfaceSegments:[segmentsX,segmentsY],sparkleCount,rockCount,rockCastShadows:!practical,reflectionStrength:uniforms.uReflectionStrength.value,waveStrength:uniforms.uWaveStrength.value,causticStrength:uniforms.uCausticStrength.value,foamStrength:uniforms.uFoamStrength.value},intent:'depth-banded turquoise river with restrained moving wave reflection, non-emissive shallow caustics, broken shoreline foam and grounded rocks; avoids flat blue plate and excessive glow',update:t=>uniforms.uTime.value=t};
  return g;
}
