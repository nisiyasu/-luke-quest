...
function sparkleField(width,depth,seed=1,count=380,size=.075,opacity=.9){const r=rng(seed),g=new THREE.BufferGeometry(),positions=[],colors=[];const cs=[new THREE.Color(0xffffff),new THREE.Color(0xd5ffff),new THREE.Color(0x8fd9f5),new THREE.Color(0xffffff),new THREE.Color(0x76b9e7)];for(let i=0;i<count;i++){positions.push((r()-.5)*width,.05,(r()-.5)*depth);const c=cs[i%cs.length];colors.push(c.r,c.g,c.b)}g.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));g.setAttribute('color',new THREE.Float32BufferAttribute(colors,3));return new THREE.Points(g,new THREE.PointsMaterial({size,vertexColors:true,transparent:true,opacity,depthWrite:false,sizeAttenuation:true}))}
function foamRing(radius=.45){const geo=new THREE.RingGeometry(radius*.98,radius*1.28,28,1);const mat=new THREE.MeshBasicMaterial({color:0xf6ffff,transparent:true,opacity:.48,depthWrite:false,side:THREE.DoubleSide});const m=new THREE.Mesh(geo,mat);m.rotation.x=-Math.PI/2;return m}
...
export function createTargetWaterV3({width=28,depth=6.2,seed=11,quality='auto'}={}){
  ...
  const uniforms={
    uTime:{value:0},
    uDeep:{value:new THREE.Color(0x022a45)},
    uMid:{value:new THREE.Color(0x075f82)},
    uShallow:{value:new THREE.Color(0x319eaa)},
    uSky:{value:new THREE.Color(0x7f9fad)},
    uSun:{value:new THREE.Color(0xffdda3)},
    uSunDir:{value:new THREE.Vector3(-.34,.88,.31).normalize()},
    uReflectionStrength:{value:practical?.15:.26},
    uSparkleStrength:{value:practical?.25:.43},
    uWaveStrength:{value:practical?.68:.94},
    uCausticStrength:{value:practical?.09:.15},
    uFoamStrength:{value:practical?.39:.58},
    uDepthContrast:{value:practical?1.0:1.18}
  };
  ...
  g.add(shoreRibbon(width,depth*.47,seed+1,practical?.36:.52),shoreRibbon(width,-depth*.47,seed+2,practical?.36:.52));
  const area=Math.max(1,width*depth),sparkCount=practical?Math.max(80,Math.floor(area*.56)):Math.max(210,Math.floor(area*1.22));
  g.add(sparkleField(width*.95,depth*.87,seed+40,sparkCount,practical?.050:.061,practical?.45:.62));
  ...
  g.userData={assetId:'water_target_v3',version:'3.8.0',stage:'M07_IN_PROGRESS',source:'repo-procedural',qualityPreset:resolvedQuality,reflectionMethod:'single-pass analytic wave-normal Fresnel sky reflection + directional sun specular; no planar RTT/SSR',performanceIntent:practical?'52x20 surface grid, reduced sparkle/caustic/shore foam, fewer rocks, no rock cast shadows':'96x40 surface grid, denser sparkle/caustic/shore foam, full shoreline rock shadows',resourceBudget:{surfaceSegments:[segmentsX,segmentsY],sparkleCount:sparkCount,rockCount,rockCastShadows:!practical,reflectionStrength:uniforms.uReflectionStrength.value,waveStrength:uniforms.uWaveStrength.value,causticStrength:uniforms.uCausticStrength.value,foamStrength:uniforms.uFoamStrength.value},intent:'target-derived deep blue-teal river with stronger basin depth, restrained moving reflection, bright broken highlights and shoreline foam, non-emissive shallow caustics, grounded rocks; avoids the prior bright cyan flat-plate read',update:t=>uniforms.uTime.value=t};
  return g;
}
