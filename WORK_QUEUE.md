...
| 3 | REQ-021 | P0 | VERIFY | 画面のどこを短くタップしてもAアクション | `requirements/REQ-021_TAP_ANYWHERE_ACTION.md` | Owner BOOT v3 exact-head re-audit complete: P0 Touch 34271984923 SUCCESS; Pages 34271984980 SUCCESS; Render 34271984938 SUCCESS。IOS_PHYSICAL_VERIFICATION=PENDING。 |
...
| 5 | REQ-001 | P0 | VERIFY | 画面任意位置Dynamic Touch Controller | `requirements/REQ-001_DYNAMIC_TOUCH_CONTROLLER.md` | Owner BOOT v3 fresh re-audit: v1.9 unified pointer authority; exact-head P0 Touch 34271984923 SUCCESS including drag/hold/direction/cancel/transition/multitouch/viewport gates。Pages 34271984980 SUCCESS。IOS_PHYSICAL_VERIFICATION=PENDING |
| 6 | REQ-023 | P0 | VERIFY | 北の退避路・進行必須手掛かりの導線修正 | `requirements/REQ-023_EVAC_ROUTE_CRITICAL_GUIDANCE.md` | Owner BOOT v3 fresh re-audit: dedicated REQ-023 browser 34263696013 SUCCESS on latest runtime lineage; current Pages 34271984980/Render 34271984938 SUCCESS; public addon exact match。IOS_PHYSICAL_VERIFICATION=PENDING |
...
| 95 | REQ-096 | P1 | VERIFY | 風切り峠・エリアタイトル演出の連続性 | `requirements/REQ-096_WINDCUT_PASS_AREA_TITLE_CARD_CONTINUITY.md` | dedicated subtitle「北尾根へ続く風の強い高所峠」を既存area-title authorityへ接続。Pages run 34033373703 SUCCESS。IOS_PHYSICAL_VERIFICATION=PENDING。 |
...
| 126 | REQ-144 | P1 | READY | Original Generic Field NPC Art | `requirements/REQ-144_ORIGINAL_GENERIC_FIELD_NPC_ART.md` | Owner 2026-09-09 direct priority preempted this work for REQ-145 Gold Vertical Slice. Existing requirement/evidence preserved; resume after challenger lane checkpoint. |
| -4 | REQ-146 | P0 | READY | Field Camera Pullback / Reference-Quality World Framing | `requirements/REQ-146_FIELD_CAMERA_PULLBACK_REFERENCE_QUALITY.md` | Latest Owner direct priority. Three Owner-provided reference screenshots are stored under `assets/reference/owner_2026-09-09_field_scale/`. Queue registration only; implementation has not started. Select next. |
| -3 | REQ-147 | P0 | READY | Battle UX Restore / In-Battle Victory Dismiss Flow | `requirements/REQ-147_BATTLE_UX_RESTORE_IN_BATTLE_VICTORY.md` | Owner physical-play corrections: 集中斬り visible without scroll; VICTORY stays over battle scene; one dismiss also consumes Luke post-battle comment. Execute after REQ-146. |
| 127 | REQ-145 | P0 | READY | Gold Vertical Slice Continuous Quality Lane | `requirements/REQ-145_GOLD_VERTICAL_SLICE_CONTINUOUS_QUALITY_LANE.md` | Owner-preempted by REQ-146 then REQ-147. Gold checkpoint/source `3cdfe355...` is preserved; resume challenger lane after the two new Owner-direct P0 items. |
