...
| 44 | REQ-044 | P1 | VERIFY | Battle-Only Poison Save Sanitization | `requirements/REQ-044_BATTLE_ONLY_POISON_SAVE_SANITIZATION.md` | stale/legacy saveからbattle-only poisonがworldへ復元・再保存される境界をcanonical save sanitizationでhardening。Pages run 34010063196 SUCCESS。IOS_PHYSICAL_VERIFICATION=PENDING |
...
| 48 | REQ-048 | P1 | VERIFY | Autosave Pulse Progress Coverage | `requirements/REQ-048_AUTOSAVE_PULSE_PROGRESS_COVERAGE.md` | later chest / hidden-find / item-cache and herb-sample completion progress are included in the dynamic deduped autosave-feedback signature. canonical `save()` remains unchanged. Pages run 34010704113 SUCCESS. IOS_PHYSICAL_VERIFICATION=PENDING |
| 49 | REQ-049 | P1 | VERIFY | Manual Backup Corruption Hardening | `requirements/REQ-049_MANUAL_BACKUP_CORRUPTION_HARDENING.md` | malformed JSON, primitives, arrays and null are rejected before state merge. corrupt slots are visibly `INVALID BACKUP`, cannot LOAD, and retain explicit overwrite/delete recovery paths. Pages run 34011257673 SUCCESS. IOS_PHYSICAL_VERIFICATION=PENDING |
| 50 | REQ-050 | P1 | VERIFY | Manual Backup Dangerous-Key Sanitization | `requirements/REQ-050_MANUAL_BACKUP_DANGEROUS_KEY_SANITIZATION.md` | `__proto__`, `constructor`, and `prototype` own keys are removed from top-level manual state and nested flags before merge. Pages run 34011382155 SUCCESS. IOS_PHYSICAL_VERIFICATION=PENDING |
| 51 | REQ-051 | P1 | VERIFY | Manual Backup Numeric Type Hardening | `requirements/REQ-051_MANUAL_BACKUP_NUMERIC_TYPE_HARDENING.md` | present canonical numeric fields are normalized dynamically against current runtime `DEFAULT`; non-number/non-finite values fall back to canonical defaults. Pages run 34011542116 SUCCESS. IOS_PHYSICAL_VERIFICATION=PENDING |
| 52 | REQ-052 | P1 | VERIFY | Readable Normal Enemy Behavior | `requirements/REQ-052_NORMAL_ENEMY_READABLE_BEHAVIOR.md` | ordinary enemies expose readable PRESSURE / BURST / STEADY behavior while preserving canonical enemy-turn, guard, poison and boss-AI chains. Pages run 34011798629 SUCCESS. IOS_PHYSICAL_VERIFICATION=PENDING |
| 53 | REQ-053 | P1 | VERIFY | Recovery Magic Foundation | `requirements/REQ-053_RECOVERY_MAGIC_FOUNDATION.md` | `癒光` adds a 5 MP battle heal with deterministic level scaling, no poison cure, no-cost/no-turn rejection at full HP or insufficient MP, and canonical enemy response on a valid cast. Pages run 34011930589 SUCCESS. IOS_PHYSICAL_VERIFICATION=PENDING |
| 54 | REQ-054 | P1 | VERIFY | 王都アルディア・北の神殿内部 | `requirements/REQ-054_ALDIA_NORTH_TEMPLE_INTERIOR.md` | canonical North Temple setting is now a walkable Aldia interior with safe entrance/exit, attendant, prayer crystal and environmental prop. no reward, healing, progression gate or protected-canon reveal added. Pages run 34012131433 SUCCESS. IOS_PHYSICAL_VERIFICATION=PENDING |
| 55 | REQ-055 | P1 | VERIFY | Consumable Shop Sell Foundation | `requirements/REQ-055_CONSUMABLE_SHOP_SELL_FOUNDATION.md` | Aldia item shop supports one-unit selling for canonical stackable consumables: 薬草 4G and 煙玉 9G. Pages run 34013983279 SUCCESS. IOS_PHYSICAL_VERIFICATION=PENDING |
| 56 | REQ-056 | P1 | VERIFY | Base Equipment Shop Comparison | `requirements/REQ-056_BASE_EQUIPMENT_SHOP_COMPARISON.md` | base-shop 青銅の剣 / 革の旅装 show current ATK/DEF -> projected stat with signed delta before purchase; downgrades from 鉄の剣 / 補強革鎧 explicitly shown. Pages run 34014165812 SUCCESS. IOS_PHYSICAL_VERIFICATION=PENDING |
| 57 | REQ-057 | P1 | VERIFY | Stackable Shop Sell Quantity | `requirements/REQ-057_STACKABLE_SHOP_SELL_QUANTITY.md` | REQ-055 single sell authority supports explicit x1/x3 quantities for herbs and smoke bombs; x3 rejects insufficient inventory; x1 compatibility and x1/x3 buying remain intact. Pages run 34014292725 SUCCESS. IOS_PHYSICAL_VERIFICATION=PENDING |
| 58 | REQ-058 | P1 | VERIFY | Accessory Equipment Foundation | `requirements/REQ-058_ACCESSORY_EQUIPMENT_FOUNDATION.md` | real third equipment slot with `旅人の護符` (60G, DEF +1 while equipped); persistent ownership/equipped flags, shop/menu integration and delta-safe DEF reconciliation. Pages run 34014440476 SUCCESS. IOS_PHYSICAL_VERIFICATION=PENDING |
