# REQ-092 — iPhone Field UI Occlusion / Camera Framing Fix
...
## 0. ID RECOVERY / OWNER OBSERVED DEFECT

This requirement was initially registered under `REQ-085` during a concurrent queue race. Fresh repository inventory later proved `REQ-085` was already canonically occupied by `REQ-085_NORTH_CLIFF_BATTLE_BACKGROUND_CONTINUITY.md`, with REQ-086..REQ-090 also already existing and REQ-091 created concurrently for the first three UI defects. The complete Owner request is therefore recovered under the next free ID, REQ-092. No Owner requirement or valid implementation checkpoint is discarded.
...
- complete implementation including portrait zoom-out: run `34031527694` SUCCESS on `3143239f670866d3e08d3a534486ab1108d519dd`
...
`REQ-091_IPHONE_FIELD_UI_OCCLUSION_FIX.md` captured the first three defects concurrently, but did not contain the later Owner-requested zoom-out and is therefore superseded by this complete requirement.

Its valid implementation commits were preserved and reused; no code was repeated merely to satisfy metadata.

The accidental duplicate-ID file `REQ-085_IPHONE_FIELD_UI_OCCLUSION_CAMERA_FRAMING.md` is a transport/coordination artifact only and must be removed after this REQ-092 copy is safely committed. Canonical REQ-085 remains the North Cliff battle-background continuity requirement.

## 9. COMPLETION / NO-STOP

REQ-092 implementation and public automated acceptance are complete enough for VERIFY.
