...
(async()=>{
  ...
  if(!meta.title.includes('Composition Rebuild v62')) throw new Error(`unexpected title ${meta.title}`);
  fs.mkdirSync('prototypes/modern-3d/evidence',{recursive:true});
  await page.screenshot({path:'prototypes/modern-3d/evidence/m03-v62-390x844.png',fullPage:true});
  fs.writeFileSync('prototypes/modern-3d/evidence/m03-v62-runtime.json',JSON.stringify(meta,null,2));
  await browser.close();
})();
