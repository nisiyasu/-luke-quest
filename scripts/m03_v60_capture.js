...
(async()=>{
  ...
  const meta=await page.evaluate(()=>({scene:document.documentElement.dataset.lqModern3d,variant:document.documentElement.dataset.m05Variant,player:document.documentElement.dataset.m08Player,m03ci:document.documentElement.dataset.m03Ci||null,three:document.documentElement.dataset.threeRevision}));
  if(errors.length) throw new Error(errors.join('\n'));
  if(meta.variant!=='m03-quality-regate-v60-macro-reframe') throw new Error(`unexpected variant ${meta.variant}`);
