...
setTimeout(()=>{
  ...
  let blockedWithoutProof=false,enteredWithProof=false,safeSpawn=false,walkable=false,footprintInteraction=false;
  ...
  try{
    ...
    /* REQ-093 legitimately supersedes REQ-081's old 'stop at north boundary' assertion:
       canonical Action at the existing boundary now enters the next walkable checkpoint. */
    s.dialog=null;s.map='northCliffRoad';s.x=10;s.y=2;s.dir='up';render();action();
    windcutEntry=s.map==='windcutPass'&&!!MAPS.windcutPass;
    windcutSafeSpawn=s.x===10&&s.y===18&&!blocked(s.x,s.y)&&encounterGrace===5;
    ...
    windcutReturnSafe=s.map==='northCliffRoad'&&s.x===10&&s.y===2&&!blocked(s.x,s.y)&&encounterGrace===4;
    ...
    if(!(blockedWithoutProof&&enteredWithProof&&safeSpawn&&walkable&&footprintInteraction&&windcutEntry&&windcutSafeSpawn&&windcutWalkable&&windcutInteractions&&windcutBoundarySafe&&windcutReturnSafe&&encounterReuse&&saveRoundTrip&&canonSafe))throw new Error('REQ-081/093 assertion false');
    ...
    render();
    marker({blockedWithoutProof,enteredWithProof,safeSpawn,walkable,footprintInteraction,windcutEntry,windcutSafeSpawn,windcutWalkable,windcutInteractions,windcutBoundarySafe,windcutReturnSafe,encounterReuse,saveRoundTrip,canonSafe});
  }
},650);
})();
