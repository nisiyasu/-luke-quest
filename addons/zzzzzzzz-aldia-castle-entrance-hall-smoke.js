...
setTimeout(()=>{
  ...
  try{
    ...
    const boundaryInteracted=!!s.dialog&&s.dialog.name==='王城大階段';
    if(!boundaryInteracted)throw new Error('deep castle boundary failed');
    ...
    marker(true,{entered,walked,guardInteracted,propInteracted,boundaryInteracted,exited,safeSpawn});
  }catch(err){
