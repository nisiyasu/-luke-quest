...
const OUTDOOR=new Set(['town','field','forest','deepForest','mistTrail','observation','evacuation','evacRoute','cliff','cliffRoad','northCliffRoad','windcutPass','northRidgeApproach','windShelf','skylineTraverse','cloudbreakSaddle']);
...
function cloudClass(map=s.map){if(['forest','deepForest'].includes(map))return'forest';if(['mistTrail','cliff','cliffRoad','northCliffRoad','windcutPass','northRidgeApproach','windShelf','skylineTraverse','cloudbreakSaddle'].includes(map))return'mist';if(['observation','evacuation','evacRoute'].includes(map))return'hostile';return'';}
function addClouds(){
...
window.LQ_WORLD_CLOUD_STATUS={maps:[...OUTDOOR],broadLayers:2,presentationOnly:true,reducedMotion:true,hasMap:(map)=>OUTDOOR.has(map),classFor:(map)=>OUTDOOR.has(map)?cloudClass(map):null};
})();
