...
const SUB={
 town:'王国南部の城郭都市',field:'王都と魔物の森を結ぶ街道',forest:'レオンを追う最初の森',deepForest:'森の気配が濃くなる深部',mistTrail:'足跡が霧へ消える追跡路',observation:'魔王軍が何かを監視している区域',evacuation:'北へ延びる古い退避路',evacRoute:'北へ延びる古い退避路',cliff:'風の強い北方崖道',cliffRoad:'風の強い北方崖道',northCliffRoad:'退避路の先へ続く風の強い北方崖道',windcutPass:'北尾根へ続く風の強い高所峠',northRidgeApproach:'風切り峠の先へ伸びる細い北尾根',windShelf:'北尾根のさらに高みへ続く風蝕の岩棚',skylineTraverse:'雲の高さを縫って北へ伸びる細い稜線',cloudbreakSaddle:'露出した稜線の合間にある風の弱い高所鞍部',windStairRidge:'風鳴りが石段を抜ける北尾根の高所',
...
function subtitle(map){return SUB[map]||'LUKE QUEST';}
function showTitle(){
 ...
 el.innerHTML=`<strong>${areaName()}</strong><small>${subtitle(s.map)}</small>`;
...
window.LQ_AREA_TITLE_STATUS={cinematic:true,mapAware:true,interiorSubtitles:true,reducedMotion:true,windStairRidgeIntegrated:true,hasMap:(map)=>Object.prototype.hasOwnProperty.call(SUB,map),subtitle};
showTitle();
})();
