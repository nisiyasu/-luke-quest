...
/* REQ-117/119 Checkpoint B — presentation-only map depth.
   ...
   never mutated. iPhone-safe depth avoids per-tile filter/isolation layers. */
...
function injectStyle(){
  ...
  st.id=STYLE_ID;
  st.textContent=`
.tile.lqDepthTile{--lq-depth-shadow:0 0 0 transparent;--lq-edge-bottom:inset 0 0 0 transparent;box-shadow:var(--lq-depth-shadow),var(--lq-edge-bottom);isolation:auto;filter:none}
.tile.lqDepthTile.lqRaisedLower{--lq-depth-shadow:0 4px 2px rgba(2,8,14,.28)}
.tile.lqDepthTile.lqPathLower{--lq-depth-shadow:0 3px 1px rgba(3,9,14,.16)}
...
.tile.lqDepthTile.lqEdgeBottom{--lq-edge-bottom:inset 0 -4px 0 rgba(4,11,18,.105)}
...
@media(max-width:430px),(pointer:coarse){
 .tile.lqDepthTile.lqRaisedLower{--lq-depth-shadow:0 3px 1px rgba(2,8,14,.24)}
 .tile.lqDepthTile.lqPathLower{--lq-depth-shadow:0 2px 1px rgba(3,9,14,.13)}
 .tile.lqDepthTile.lqEdgeLeft::before,.tile.lqDepthTile.lqEdgeRight::after{width:3px}
}
...
function fail(reason){
  const el=document.createElement('i');el.className='lqReq117CheckpointBSmokeFailure';el.hidden=true;el.dataset.reason=String(reason);document.body.appendChild(el);
  throw new TypeError(`REQ-117 checkpoint B smoke failed: ${reason}`);
}
...
function smoke(){
  ...
  try{
    for(const map of maps){
      ...
      assert(tiles.every(t=>getComputedStyle(t).filter==='none'),`${map} filter compositor layer detected`);
      assert(tiles.every(t=>getComputedStyle(t).isolation==='auto'),`${map} isolation compositor layer detected`);
      assert([...tiles.flatMap(t=>[...t.querySelectorAll('*')])].every(el=>getComputedStyle(el).pointerEvents==='none'||!el.classList.contains('lqDepthOverlay')),`${map} depth overlay pointer safety`);
    ...
    const marker=document.createElement('i');marker.id='lqReq117CheckpointBMarker';marker.hidden=true;
    marker.dataset.status='PASS';marker.dataset.town='true';marker.dataset.field='true';marker.dataset.forest='true';marker.dataset.edges='true';marker.dataset.dropShadows='true';marker.dataset.presentationOnly='true';marker.dataset.compositorSafe='true';marker.dataset.filterEffects='false';marker.dataset.isolationLayers='false';marker.dataset.evidence=evidence.join('|');document.body.appendChild(marker);
  }finally{
...
window.LQ_REQ117_CHECKPOINT_B_STATUS={requirement:'REQ-117',checkpoint:'B',neighborAwareEdges:true,raisedLowerShadows:true,pathLowerShadows:true,mapDataMutation:false,collisionMutation:false,inputAuthority:false,compositorSafe:true,filterEffects:false,isolationLayers:false,iosPhysicalVerification:'PENDING'};
window.LQ_REQ119_CHECKPOINT_B_STATUS=window.LQ_REQ117_CHECKPOINT_B_STATUS;
window.LQ_REQ117_CHECKPOINT_B_TEST={applyDepth,smoke};
window.LQ_REQ119_CHECKPOINT_B_TEST=window.LQ_REQ117_CHECKPOINT_B_TEST;
const q=new URLSearchParams(location.search);
setTimeout(()=>{if(q.has('lqSmoke')||q.get('lqReq117DepthSmoke')==='1')smoke();},2500);
})();
