...
const OUTDOOR=new Set(['town','field','forest','deepForest','mistTrail','observation','evacuation','evacRoute','cliff','cliffRoad','northCliffRoad','windcutPass','northRidgeApproach','windShelf','skylineTraverse','cloudbreakSaddle','windStairRidge']);
let last={map:s.map,x:s.x,y:s.y};
function kindFor(map=s.map){if(['forest','deepForest'].includes(map))return'leaf';if(['mistTrail','evacuation','evacRoute','cliff','cliffRoad','northCliffRoad','windcutPass','northRidgeApproach','windShelf','skylineTraverse','cloudbreakSaddle','windStairRidge'].includes(map))return'mist';if(map==='observation')return'ash';return'dust';}
function stepFx(){
...
window.LQ_FOOTSTEP_PARTICLE_STATUS={terrainAware:true,presentationOnly:true,reducedMotion:true,mapKeyCoverage:[...OUTDOOR],hasMap:(map)=>OUTDOOR.has(map),kindFor:(map)=>OUTDOOR.has(map)?kindFor(map):null,windStairRidgeIntegrated:true};
})();
