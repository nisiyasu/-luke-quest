...
const OUTDOOR=new Set(['town','field','forest','deepForest','mistTrail','observation','evacuation','evacRoute','cliff','cliffRoad']);
let last={map:s.map,x:s.x,y:s.y};
function kind(){if(['forest','deepForest'].includes(s.map))return'leaf';if(['mistTrail','evacuation','evacRoute','cliff','cliffRoad'].includes(s.map))return'mist';if(s.map==='observation')return'ash';return'dust';}
function stepFx(){
 ...
 const shell=app.querySelector('.gameShell');if(!shell)return;const fx=document.createElement('div');fx.className=`lqStepFx ${kind()}`;shell.appendChild(fx);setTimeout(()=>fx.remove(),520);
}
const renderBase=render;render=function(){const r=renderBase();stepFx();return r;};
window.LQ_FOOTSTEP_PARTICLE_STATUS={terrainAware:true,presentationOnly:true,reducedMotion:true,mapKeyCoverage:[...OUTDOOR]};
})();
