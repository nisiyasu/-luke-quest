...
const OUTDOOR=new Set(['town','field','forest','deepForest','mistTrail','observation','evacRoute','cliffRoad']);
let last={map:s.map,x:s.x,y:s.y};
function kind(){if(['forest','deepForest'].includes(s.map))return'leaf';if(['mistTrail','evacRoute','cliffRoad'].includes(s.map))return'mist';if(s.map==='observation')return'ash';return'dust';}
function stepFx(){
...
window.LQ_FOOTSTEP_PARTICLE_STATUS={terrainAware:true,presentationOnly:true,reducedMotion:true};
})();
