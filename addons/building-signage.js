...
function decorate(){
 ...
 for(const el of app.querySelectorAll('.lqBuildingDoor')){
   if(el.querySelector('.lqBuildingSign'))continue;
   let found=null;
   for(const [cls,data] of Object.entries(SIGNS))if(el.classList.contains(cls)){found=data;break;}
   if(!found)continue;
   const e=document.createElement('span');e.className='lqBuildingSign';e.innerHTML=`${found[0]}<small>${found[1]}</small>`;el.appendChild(e);
 }
}
const worldB=world;world=function(){worldB();decorate();};
const renderB=render;render=function(){const r=renderB();decorate();return r;};
window.LQ_BUILDING_SIGN_STATUS={inn:true,shop:true,temple:true,residence:true};
decorate();
})();
