...
style.textContent=`
.lqAldiaTempleFront{position:absolute;z-index:4;pointer-events:none;border:2px solid #d5d7d0;border-radius:5px 5px 2px 2px;background:linear-gradient(180deg,#fff8df22 0 12%,transparent 13% 76%,#342f2a33 77%),linear-gradient(90deg,#766f63,#b5aa91 18%,#d1c7ae 50%,#a79b82 82%,#665f55);box-shadow:inset 0 2px #f7eedb66,inset 0 -10px #554f4744,0 10px 12px #0006}
...
.lqAldiaTempleFront:after{content:'✦';position:absolute;left:50%;top:13px;transform:translateX(-50%);width:32px;height:32px;border:3px solid #d9c66d;border-radius:50%;display:grid;place-items:center;background:radial-gradient(circle at 42% 35%,#ffffff 0 10%,#dff6ff 11% 28%,#7fb4d5 29% 55%,#274d74 56%);color:#ffe58a;font-size:17px;text-shadow:0 0 5px #fff1a5;box-shadow:0 0 0 2px #78652a66,0 0 12px #91c6ff88,inset 0 0 8px #e8fbff99}
.lqAldiaTempleStep{position:absolute;z-index:4;pointer-events:none;border:1px solid #857d70;border-radius:2px;background:linear-gradient(180deg,#d7cfbd 0 35%,#a69d8b 36% 67%,#6f685f 68%);box-shadow:0 4px 4px #0005,inset 0 1px #fff4db66}
.lqAldiaFountain{position:absolute;z-index:4;pointer-events:none;width:76px;height:44px;border-radius:50%;border:5px solid #b8b19d;background:radial-gradient(ellipse at 50% 42%,#f1fdff 0 10%,#bcebf3 11% 22%,#73bed0 23% 42%,#3f819b 43% 54%,#294f68 55% 61%,#a9a18f 62% 69%,#777063 70%);box-shadow:0 8px 9px #0007,inset 0 3px #fff8e5,inset 0 -5px #334c5b55}
...
.lqAldiaMarketAwning{position:absolute;z-index:4;pointer-events:none;width:82px;height:34px;border:2px solid #59493a;border-radius:5px;background:linear-gradient(180deg,#fff5d8aa 0 18%,transparent 19% 100%),repeating-linear-gradient(90deg,#f3dfb4 0 13px,#28558e 13px 26px);box-shadow:0 8px 8px #0007,inset 0 -5px #5a3f2a33}
.lqAldiaMarketAwning:after{content:'';position:absolute;left:7px;right:7px;bottom:-19px;height:19px;border-left:5px solid #614d38;border-right:5px solid #614d38;box-shadow:2px 3px 2px #0005,inset 2px 0 #a47a4a55,inset -2px 0 #35271d55}
.lqAldiaBench{position:absolute;z-index:4;pointer-events:none;width:58px;height:20px;border-radius:4px;background:linear-gradient(180deg,#b58a5d 0 22%,#98714c 23% 56%,#5f452f 57%);border:2px solid #4a3b2e;box-shadow:0 6px 5px #0005,inset 0 2px #d6ad7a66,inset 0 -4px #3c2d2233}
...
function decorate(){
  ...
  add(w,'lqAldiaTempleStep',6.55,8.82,2.9,.28);
...
window.LQ_ALDIA_LANDMARK_STATUS={checkpoint:'B',presentationOnly:true,collisionChanged:false,temple:1,templeSteps:1,fountain:1,marketAwnings:2,benches:4,crates:3,barrels:3};
})();