...
/* Collision-safe add-on: first status ailment, battle-only poison.
   Poison cannot reduce Luke below 1 HP and is cleared whenever battle ends. */
const POISON_ENEMIES=new Set(['森グモ','黒甲ムカデ','霧喰いヤマネコ']);
const POISON_CHANCE=.18,POISON_DAMAGE=2,POISON_TURNS=3;

function normalizedPoisonValue(status,screen){
 const raw=status&&typeof status==='object'&&!Array.isArray(status)?status.poison:0;
 const turns=Math.max(0,Math.floor(Number(raw)||0));
 return screen==='battle'?turns:0;
}
function normalizePoisonState(){
 if(!s.status||typeof s.status!=='object'||Array.isArray(s.status))s.status={};
 s.status.poison=normalizedPoisonValue(s.status,s.screen);
 return s.status;
}
normalizePoisonState();
...
function clearPoison(){normalizePoisonState();s.status.poison=0;}
...
function sanitizeBattleOnlyPoison(){const before=s.status?.poison;normalizePoisonState();return before!==s.status.poison;}
...
const savePoisonBase=save;save=function(...args){normalizePoisonState();return savePoisonBase.apply(this,args);};
normalizePoisonState();
...
const potionPoisonBase=potion;potion=function(){normalizePoisonState();const cured=s.status.poison>0&&(s.potions||0)>0;if(cured)clearPoison();const r=potionPoisonBase();if(cured&&s.screen==='battle'){s.log.push('薬草で毒も消えた。');battle();}return r;};
if(window.lqUseFieldHerb){const herbFieldBase=window.lqUseFieldHerb;window.lqUseFieldHerb=function(){normalizePoisonState();const cured=s.status.poison>0&&(s.potions||0)>0;if(cured)clearPoison();return herbFieldBase();};}
...
const battlePoisonBase=battle;battle=function(){normalizePoisonState();const r=battlePoisonBase();addPoisonUi();return r;};
const renderPoisonBase=render;render=function(){normalizePoisonState();const r=renderPoisonBase();addPoisonUi();return r;};
window.LQ_STATUS_AILMENT_STATUS={
 poison:{
  ...
  migration:{ensuresStatusObject:true,normalizesMalformedPoison:true,preservesOtherStatusFields:true},
  battleEnded,shouldSanitizePoison,normalizedPoisonValue
 }
};
save();addPoisonUi();
})();
