...
function optionalText(){if(s.flags?.elderCharmQuest&&!s.flags?.elderCharmComplete)return s.flags.elderCharmFound?'銀留め具を老人へ返す':'王都近郊で銀留め具を探す';if(s.flags?.forestBountyAccepted&&!s.flags?.forestBountyComplete)return s.forestBountyKills>=3?'討伐掲示板で報酬を受け取る':`森の討伐 ${s.forestBountyKills||0}/3`;return'';}
...
const worldO=world;world=function(){worldO();addChip();};const renderO=render;render=function(){const r=renderO();addChip();return r;};window.LQ_OPTIONAL_OBJECTIVE_STATUS={visible:true};if(s.screen==='world')addChip();
})();
