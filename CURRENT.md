...
- UPDATED_AT: `2026-09-08 11:29 JST`
- REPOSITORY: `nisiyasu/-luke-quest`
- ACTIVE_BRANCH: `main`
- LATEST_IMPLEMENTATION_COMMIT_SHA: `2a467bb292e042dad668a65f5eb1dd8d80f2b25d`
...
- CURRENT_BUILD_STATUS: `REQ-127 remains P0 IN_PROGRESS. Production resume-heal v1.2 adds window focus as a third presentation-only foreground recovery boundary alongside pageshow and visibilitychange; gameplay/save state authority is unchanged. Regression checkpoint 6839420b... deliberately poisons the world with frozen lqMapArrive + full-screen black fade + hidden/zero-size world, then proves that dispatching focus alone invokes the canonical heal and restores the presentation without logical-state mutation. Pages 34180095109 SUCCESS; Render Liveness 34180095133 SUCCESS including Chromium + Playwright WebKit iPhone-sized pixel analysis; cache-busted public recovery 34180166009 SUCCESS. Physical iPhone result remains unresolved; IOS_PHYSICAL_VERIFICATION=PENDING.`
...
- RECENT_CHECKPOINTS: `6e50e93a... adds presentation-only PWA resume world-plane heal; d57783b2... upgrades it to clear frozen lqMapArrive 0% presentation state; a8611ba3... deliberately reproduces frozen arrival + dark fade + stale world plane; c402c3bb... requires the same recovery in WebKit; cf544fa3... fail-closes Render Liveness; c26b67b0... removes addon load-order race; 967408ad... stabilizes smoke frame settling; 2a467bb2... adds presentation-only window-focus recovery and resume-heal contract v1.2; 6839420b... proves focus-event-only recovery from deliberately poisoned world presentation.`
- TESTS_AND_VERIFICATION: `Pages 34180095109 SUCCESS including real Pages deploy; Render Liveness 34180095133 SUCCESS including exact iPhone-sized Chromium capture/pixels plus Playwright WebKit iPhone-sized capture/pixels; cache-busted recovery 34180166009 SUCCESS on head 6839420b...; focus smoke requires canonical heal invocation, fade/arrival cleanup, shell/world/player visibility, expected world geometry, PASS marker and unchanged logical gameplay state. Prior stabilized lanes remain preserved.`
- KNOWN_ISSUES: `Owner physical iPhone Home Screen PWA was still dark/black before the frozen-arrival and focus-recovery deployments. Frozen lqMapArrive 0% presentation plus incomplete foreground lifecycle recovery remain evidence-backed candidates, but physical causality is not yet proven. IOS_PHYSICAL_VERIFICATION=PENDING.`
- BLOCKERS: `No machine-side blocker. REQ-127 v1.2 production candidate and validation lanes are machine/public green; physical iPhone confirmation remains Owner-only. Continue evidence-based lifecycle/occluder/compositor diagnosis if the symptom persists.`
- NEXT_ACTION: `Keep REQ-127 active until Owner physical verification. If the device remains dark after v1.2 is live, inspect fresh iPhone-only full-viewport occluder/compositor/lifecycle evidence; do not repeat already disproven rollback/cache/gamepad/ambient/generic compositor/fade-only paths or re-add the now-covered focus boundary without new evidence.`
...
- DO_NOT_REPEAT: `Do not claim physical iPhone PASS from CI. Do not re-run already-failed broad source rollback, service-worker/cache-only purge, gamepad rollback, ambient-audio rollback, generic compositor-safe-mode, or fade-only fixes without new evidence. Do not allow decorative lqMapArrive animation-fill-mode:both to remain a lifecycle recovery authority. Do not weaken the deliberate frozen-arrival/focus render smoke merely to obtain green. Window focus is now an explicit canonical resume-heal boundary and machine-proved; do not add a parallel focus recovery handler. Preserve P0 canonical touch/fullscreen contracts.`
...
- STATUS: `IN_PROGRESS / OWNER PHYSICAL BLACK-SCREEN REPORT REOPENED / FROZEN-ARRIVAL + FOCUS RECOVERY MACHINE+PUBLIC GREEN / IOS_PHYSICAL_VERIFICATION=PENDING`.
...
- Production checkpoint `2a467bb292e042dad668a65f5eb1dd8d80f2b25d` upgrades resume-heal to `1.2.0` and adds `window focus` as an additional presentation-only recovery boundary. It reuses the same canonical heal path; it does not add gameplay/input/save mutation.
- Regression checkpoint `6839420b34d304bec823bdb1c30a463df52edbd5` poisons the world a second time after the direct-heal proof, dispatches only `window focus`, and requires healCount/reason evidence plus fade/arrival cleanup, shell/world/player visibility, restored geometry and unchanged canonical logical state.
- On `6839420b...`, Standard Pages `34180095109`, Render Liveness `34180095133` (Chromium + WebKit iPhone-sized pixel analysis), and cache-busted public recovery `34180166009` are SUCCESS.
- This extends lifecycle recovery coverage; it does not establish physical iPhone causality or PASS.
